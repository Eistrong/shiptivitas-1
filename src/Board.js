...
export default class Board extends React.Component {
  ...
  changeCard(card, target) {
    // change column name
    // changecolor based on new column
    if (target === "Backlog") {
      card.status = this.swimlanes.backlog;
      card.className = "Card Card-grey";
    } else if (target === "In Progress") {
      card.status = this.swimlanes.inProgress;
      card.className = "Card Card-blue";
    } else {
      card.status = this.swimlanes.complete;
      card.className = "Card Card-green";
    }
  }

  componentDidMount() {
    this.drake.containers.push(this.swimlanes.backlog.current);
    this.drake.containers.push(this.swimlanes.inProgress.current);
    this.drake.containers.push(this.swimlanes.complete.current);
    this.drake.on("drop", (el, target) =>
      this.changeCard(el, target.previousSibling.innerHTML)
    );
  }
}
